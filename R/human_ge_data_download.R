...
for (gse in geo_ids){
  try({
    geo_objs[[gse]] = getGEO(gse,destdir = paste(WD,GEO_destdir),GSEMatrix = T,getGPL = F)
  })
}
# failed downloads:
...
gsms = metadata[,1]
sra_gsms = union(acute_metadata[grepl("SRA",acute_metadata[,3]),1],longterm_metadata[grepl("SRA",longterm_metadata[,3]),1])
